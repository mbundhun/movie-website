...
const MovieDetail = () => {
  ...
  const calculateAverageRating = () => {
    ...
    const sum = reviews.reduce((acc, review) => {
      // Calculate average of three ratings if available, otherwise use overall rating
      if (review.performance_rating && review.directing_rating && review.screenplay_rating) {
        const avg = (review.performance_rating + review.directing_rating + review.screenplay_rating) / 3;
        return acc + avg;
      }
